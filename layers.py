...
from layer_utils import PConv, PCF, FasterPConv, index_points, VI_coordinate_transform, Linear_BN, UnaryBlock
...
class PointTransformerLayer(nn.Module):
    ...
    def forward(self, xyz, feats, nei_ind, sparse_xyz=None, inv_neighbors=None, inv_k=None, inv_idx=None) -> torch.Tensor:
...
class PointConvStridePE(nn.Module):
    ...
    def forward(
        ...
        # if True:
            ...
            new_feat = FasterPConv.forward(feats_x, inv_neighbors=inv_neighbors, inv_k=inv_k, inv_idx=inv_idx, neighbor_inds=nei_inds, weightnet=weights, additional_features=feat_pe)
            # test_prit = 10
        else:
