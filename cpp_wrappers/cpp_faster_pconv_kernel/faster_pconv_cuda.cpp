...
std::vector<torch::Tensor> pconv_backward_opt(
    torch::Tensor grad_output,
    torch::Tensor input,
    torch::Tensor inverse_neighbor,
    torch::Tensor inverse_neighbor_k,
    ...
    torch::Tensor additional_features)
{
    ...
    CHECK_INPUT(weights);
    CHECK_INPUT(additional_features);
    return pconv_cuda_backward_opt(grad_output,input, inverse_neighbor, inverse_neighbor_k, inverse_neighbor_idx, neighbor_inds, weights,additional_features);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("pconv_backward_opt", &pconv_backward_opt, "PointConv backward opt (CUDA)");
}
